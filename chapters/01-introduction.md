# 1 Introduction

## 1.1 Sword & Sorcery

The term _sword & sorcery_ was coined by Fritz Leiber in 1961, though the form was already three decades old - born in the pages of _Weird Tales_ with the works of Robert E. Howard and his creations: Kull, Bran Mak Morn, Solomon Kane, and ultimately Conan the Cimmerian. Leiber, whose own Fafhrd and the Grey Mouser stories define the urban-rogue side of the genre, later put it as well as anyone: his heroes were _"almost at the opposite extreme from the heroes of Tolkien."_

These are adventure stories of larger-than-life protagonists - thieves, mercenaries, barbarians - driven not by destiny or quest but by their own roguish desires. They swagger across a world already ancient, lost to us in the depths of time, pitting their cunning and their blades against dark sorcerers, cosmic horrors, ancient things in swamp-drowned temples, knife-eyed cults, and maniacal kings - in search of gold and glory.

Sorcery, while prevalent in these stories, is rarely a tool the heroes can use. It is more often the obstacle - an inheritance from a deeper past, or a transaction with things that should not be spoken to. The genre answers cosmic dread not with despair but with action: the swing of a blade against the impossible, the escape with a cursed book and the rest of one's life to run with it.

## 1.2 What Warrior \* Sorcerer \* Rogue Is

WSR is a tabletop roleplaying game built on Chaosium's **Basic Roleplaying Universal Game Engine (BRP UGE)**, released under the ORC License.

### 1.2.1 Characters

Seven characteristics define your character: STR, CON, SIZ, INT, POW, DEX, CHA. From these derive your Hit Points (CON + SIZ / 2), your Power Point pool (= POW), your damage modifier, and your skill category bonuses. Choose a Cultural Background (§2.4) and a Profession (§2.5) to gain starting skills. Spend INT x10 free points on any skills you like. Choose 2 Talents from the four talent tables (§2.7). Write down 3 Passions: one at 80%, two at 60%.

### 1.2.2 Action Resolution

Characters are described primarily by skills, expressed as percentile ratings. When something uncertain and meaningful is at stake, roll two d10 as a percentile number (01-00); roll equal to or less than your skill to succeed. Your GM sets difficulty: Easy doubles the effective skill, Difficult halves it.

- **Critical Success** - a roll of 01, or a success where both dice show the same digit (11, 22, 33, 44, 55, 66, 77, 88, 99). The best possible result.
- **Success** - roll equals or beats the target; dice do not match.
- **Failure** - roll exceeds the target; dice do not match.
- **Fumble** - roll is 00 (always bad), OR the roll exceeds your target and the dice match. Something has gone wrong.

If you rolled an ordinary success, you can spend Power Points to convert it to a Critical. Find the nearest matching-dice value below your roll that is still within your skill. The cost is the gap between your roll and that matching value.

### 1.2.3 Power Points

Each character's POW characteristic defines their Power Point pool - their vital force, their luck, and for sorcerers, their fuel for magic. PP are spent on Fate Point options (re-rolling, deflecting killing blows, standing your ground under wounds), on converting near-misses into criticals, and on spell casting.

### 1.2.4 Advancement

When you _successfully_ use a skill in a genuinely uncertain, dramatic situation, mark it for an experience roll at the end of the adventure. Failure earns nothing - the genre learns from what works. (Sustained training or immersive practice can earn a checkmark without a roll; see §5.7.) At the end of the adventure, roll D100; add INT/2. If the result exceeds your current skill rating, the skill improves by 1D6.
