...
GM reference for environmental threats, natural hazards, and situation rules outside ordinary combat. Player-facing rules (prone, aimed attacks, disengaging, close combat, weapon length) are in §6.
...
| Alchemical acid             | varies | Fast  | HP damage to flesh; armour degraded 1 point per round of contact            | See §5 for full fire/acid rules. POT equals the 1D6 type used.                               |
...
## 7.3 Herbs, Drugs, and Substances
...
Craft (Apothecary) (§3.4.8) prepares and recognises these substances. Medicine (§3.4.25) administers them. Most use the Resistance Table - POT vs CON - the same as the poison system in §7.2. Effects are time-limited unless noted; some substances are addictive (see §7.3.3).
...
### 7.3.2 Drugs and Strange Substances

These produce intentional effects: alertness, fearlessness, dreams, hallucinations. Most are addictive (see §7.3.3).
...
| Lotus Dust                | Inhaled or eaten. Target makes POT 12 vs CON; failure means deep sleep for 1D6 hours - matching the Slumber of the Lotus spell (§4). The sleeper does not wake to ordinary noise. Strongly addictive after three uses.                                                                                   | Lotus harvest; uncommon     | Average   |
...
| Smoke of the Silver Lotus | Inhaled by sorcerers to force the daytime trance (§4.2.4). A small dose opens the threshold long enough to swap a single loaded spell; the user is unaware of the physical world for the duration. Strongly addictive after three uses. Restricted; mere possession is a capital offence in some cities. | Pale highland lotus; rare   | Expensive |
...
| Building fire (room fully alight)                    | 2D6 HP           | Plus smoke inhalation (see §5.2). Character automatically catches fire unless armoured. |
...
### 7.7.2 Drowning and Asphyxiation
...
**Smoke:** 1D4 HP per round and the escalating Stamina roll above. Dense smoke: 1D6 HP per round (see §5.2).
...
### 7.10.1 Describing Position in Prose
...
### 7.10.2 Adjudicating Range Bands

The four range bands - Near, Normal, Far, Extreme - are defined in §6.3.8 and give missile weapons a functional range structure without requiring measurement. At the table, apply them as follows.
...
### 7.10.3 Keeping Tactical Pace Without a Grid
...
## 7.11 Group Stealth and Concealment
...
### 7.11.1 Exceptions and Refinements
...
### 7.11.2 Concealment for Stationary Groups
