...
3. **Action.** Combatants act in descending initiative order. On their Initiative Rank, a character takes one action (see §6.2 Combat Actions). Parries and dodges are reactions, not actions, and happen in response to incoming attacks regardless of initiative.
...
**Multiple actions in a round.** A character with a weapon skill above 100% (§5.1.4) or other circumstance permitting a second action takes the second action 5 Initiative Ranks below the first.
...
- **Attack.** Make one melee or missile attack against a target in range (see §6.3).
- **Cast a spell.** The spell resolves on the caster's Initiative Rank (which they may have rolled with INT instead of DEX; see §6.1). See §4 for sorcery rules.
- **Move.** See §6.2.1 below.
- **Engage.** Close to hand-to-hand range with a chosen opponent and make a single attack at no penalty - a combined move-and-attack into close combat. See §6.2.2.
- **Disengage.** Break out of close combat. See §6.2.3.
...
- **Parry or Dodge.** React to an incoming attack with a defensive roll (§6.3.3, §6.3.4). No declaration required.
- **Fight defensively.** Declare instead of attacking; gain one free dodge that does not count against the cumulative parry/dodge penalty stack (§6.3.7).
...
2. **Knocked-back disengage.** If a character has been Knocked Down (§6.5.2), they may attempt a Dodge immediately. Success: the character rolls away and is disengaged. Failure: they remain prone and engaged.
...
## 6.3 Attack and Defence

### 6.3.1 The Attack Roll

To attack, roll D100 against the relevant weapon skill. Consult the outcome tiers from §5.2:

- **Critical Success** (01 or matching dice within success range): the attack lands with exceptional force. Consult the appropriate crit table in §A1. The attacker may also have spent PP to convert a normal success to a critical (see §5.6.1).
...
### 6.3.2 The Attack and Defence Matrix
...
The exception is Fight Defensively (§6.3.7). A character in that stance may freely mix parries and dodges, and the cumulative -30% penalty stack applies to both together: the second defensive action of the round (parry or dodge) is at -30%, the third at -60%, regardless of which type was first.
...
### 6.3.6 Critical Attacks and Armour
...
- They may freely mix parries and dodges in the same round, sharing a single cumulative -30% penalty stack (the exception in §6.3.5).
...
### 6.3.8 Missile Weapons

Missile attacks follow the same success tiers as melee attacks. Missiles cannot generally be dodged or parried by the target unless the attack was visible before it was launched (the first visible arrow or thrown weapon in a round may be dodged at Difficult). A shield can intercept incoming missiles; see §6.4.3.

**Range bands.** Range affects all missile attacks:

| Range                        | Modifier                                                                                                                                                                                                               |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Near (within arm's reach)    | Easy to hit, but: short bows are Difficult, longbows and composite bows are Impossible (too long to bring to bear); thrown weapons can't be thrown and become improvised melee weapons (use §6.7.8 Improvised Weapons) |
...
A character with weapon skill above 100% who also volleys combines volley fire with Multiple Attacks (§6.3.9): each shot is Difficult _and_ uses the split skill.

## 6.4 Damage and Armour
...
**Parry.** Shield skill is used to parry melee attacks, with the same -30% cumulative penalty as all parry skill uses (§6.3.3).
...
**Reeling Blows.** A non-critical attack that deals damage equal to or greater than the defender's **half max HP** in a single blow (after armour) inflicts the **Reeling** status (§6.5.2). The attacker has the advantage on the next exchange.
...
Triggers: Entangle attack (§6.10.4); sorcery (Inescapable Bonds and similar); snares and traps.
...
Triggers: Grapple skill (§6.7.7); Grapple criticals (per §A1.6 entries).
...
Triggers: Pierce criticals (per §A1.3 entries); set weapon against a charge (§6.8); falling onto spikes.
...
- Rising costs the character's Initiative Rank (a full action). Alternately, they may attempt a Difficult Dodge on their Rank to rise without consuming it (§6.6.8).

Triggers: Krush criticals (per §A1.4 entries); failed DEX roll after Knockback; failed Agility roll on a slippery surface (§6.6.9); certain bestiary attacks.
...
Trigger: a non-critical attack that deals damage equal to or greater than the defender's half max HP in a single blow (after armour). See §6.5.1.
...
Triggers: Krush criticals (per §A1.4 entries); Knockout attacks (§6.7.7); pain from removing an embedded weapon (see Impaled); some bestiary special attacks.
...
### 6.6.2 Backstabs and Helpless Opponents
...
### 6.6.5 Close Combat and Weapon Length
...
**Fortified positions.** A character standing or kneeling behind a sturdy defensive structure (a stone parapet, a battlement, the lip of a trench) is in a fortified position. Attacks against them are Difficult, as with cover. A character who crouches or kneels _entirely_ behind the fortification cannot be hit by small missile fire at all - though they remain vulnerable to area attacks, siege weapons, and sorcery that reaches over or through. Aimed Attacks (§6.7.1) can overcome a fortified position one accumulated Rank at a time.

### 6.6.8 Fighting While Down
...
### 6.6.9 Slippery and Unstable Surfaces

Fighting on ice, oil, wet stone, a ship's deck in a storm, or any other unsteady footing requires an Agility roll (DEX x5) at the start of each round to keep balance. On a failure, the character falls prone (§6.6.8). On a fumble, they take 1D3 damage from the fall.
...
Picking up a dropped weapon takes a Non-combat action (§6.2) and requires being unengaged. A weapon within arm's reach may be retrieved by a Short Move combined with the Non-combat action on the same Initiative Rank.

### 6.7.3 Two-Weapon Fighting
...
**Off-hand parries.** A weapon held in the off-hand may parry attacks at no off-hand penalty. The cumulative parry penalty stack (§6.3.3) still applies across all parry attempts in the round.

**Compatible weapons.** Both weapons must be one-handed (1H, or 1H/2H wielded one-handed). A shield wielded in the off-hand is for parrying or shield-bash (§6.4.3) only and does not enable an off-hand attack roll alongside the primary.

### 6.7.4 Quickdraw

Drawing a weapon is normally a Non-combat action (§6.2), unavailable to engaged characters. A character may instead **quickdraw**: draw the weapon and attack in the same Initiative Rank, at **-20%** to the attack roll. Available only with weapons designed to be carried ready (a sheathed sword, dagger, hand-axe, throwing knife) - not a stowed bow or bundled spear.

A character surprised and reaching for their weapon (§6.6.1) may quickdraw as their first action; they are otherwise treated as surprised for that exchange.

### 6.7.5 Sweep and Area Attacks
...
Aiming reduces the risk: each +10% from Aimed Attacks (§6.7.1) also reduces the 1D6 friendly-fire range by 1 (so a single +10% aim means friendly fire on 1 only; +20% removes it entirely). A character who declares "I'll wait for a clear shot" delays their initiative until the melee separates.

### 6.7.7 Knockout and Subdual
...
Attack skill is **half the character's nearest similar weapon skill** (e.g., Club for a chair leg, Dagger for a sharp shard) or 25%, whichever is higher. Improvised weapons take 1 HP damage per successful parry and may break (§6.4.4).
...
Cutting wounds. Slash criticals resolve on the **§A1 Slash table** (§A1.2) and characteristically inflict the **Bleeding** status (§6.5.2) on top of the wound itself.

### 6.10.2 Impaling (spears, arrows, javelins, thrusting swords)

Puncturing wounds. Impale criticals resolve on the **§A1 Pierce table** (§A1.3) and characteristically inflict the **Impaled** status (§6.5.2) - the weapon lodges in the target until pulled free.

### 6.10.3 Crushing (maces, hammers, clubs, shields)

Blunt-force wounds. Crush criticals resolve on the **§A1 Krush table** (§A1.4) and characteristically inflict **Knockback**, **Knocked Down**, or **Stunned** status (§6.5.2) depending on the entry.
...
### 6.10.5 Burn (sorcerous flame, dragon breath, alchemical acid)

Heat and flame wounds. Burn criticals resolve on the **§A1 Burn table** (§A1.5) and characteristically inflict the **Burning** status (§6.5.2). Following the same band structure (A through E) and matching-dice severity rules as the other damage types.

### 6.10.6 Grapple (Grapple skill, constricting attacks)

Holds and crushing grips. Grapple criticals resolve on the **§A1 Grapple table** (§A1.6) and characteristically inflict the **Grappled** status (§6.5.2) - an active hold from a thinking opponent (compare to Entangled, which is escape from a passive constraint).
...
A Dying Blow cannot be a defensive action. It can trigger a Fate Point spend, including Deflect the Killing Blow (see §5.6.8).
