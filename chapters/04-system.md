...
**What's elsewhere.** The skill catalogue is in §3. Combat mechanics are in §5. Sorcery is in §7. The physiological and situational rules - movement and chases, encumbrance, fatigue, hunger, disease, poison, drugs, fire, cold, falling, drowning, mass combat, group stealth - live in §6 Spot Rules and §8 Equipment (§8.11 encumbrance).

## 4.1 The Percentile Roll
...
### 4.1.4 Skill Ratings Above 100%

Through experience improvement (§4.7) and characteristic bonuses, a skill rating can grow past 100%. WSR PCs reach this territory in their core specialities by mid-career; lifelong masters and named NPCs can rise higher still.

The percentile roll mechanics continue to work normally - any roll equal to or below the rating succeeds, 00 is always a Fumble, the matching-dice rule still applies (§4.2.2) - so the practical effects of a rating above 100% are:
...
### 4.2.2 Matching Dice and Skill Level
...
### 4.2.4 Non-Combat Critical Outcomes
...
## 4.3 Special Skill Rolls

### 4.3.1 Opposed Skill Rolls
...
### 4.3.3 Complementary Skills and Augments
...
### 4.5.4 What Spends Power Points

Power Points are spent by:

- **Sorcery (§7.2).** Casting spells costs PP equal to the spell's power cost, paid at the moment of casting.
- **Buying a Critical (§4.6.1).** A Fate Point spend with variable cost: converting an ordinary success to a Critical costs PP equal to the gap between the roll and the nearest lower matching value.
- **Other Fate Point spends (§4.6).** Fixed-cost spends from the menu - re-rolls, defensive responses, forcing a success.
...
## 4.6 Fate Point Spends

Fate Point spends are PP expenditures that bend the dice or the fiction in the character's favour. Every spend below draws from the same PP pool that fuels sorcery and talent activations (§4.5).
...
### 4.6.1 Buying a Critical (Cost: variable)
...
>
> **Example (non-combat):** A character with Medicine 55 treats a wounded comrade. They roll 41 - an ordinary success (heals 1D4 HP) - but want the Critical to heal 2D4 HP or restore a lost characteristic point (§4.2.4). Matching values within skill: 11, 22, 33, 44, 55. Costs: 30, 19, 8, 3, 14 PP. They claim 44 for 3 PP. Non-combat criticals have no severity bands; the cheapest matching value is always the right pick.
...
- A roll forced to success by Push Through (§4.6.6) is not eligible for a Critical Buy.
- A roll that has been re-rolled (§4.6.2) is not eligible for a Critical Buy.

Sorcerer PCs: every PP spent on a Critical Buy is a PP unavailable for casting. See §7.3.1.

### 4.6.2 Re-roll (Cost: 5 PP)
...
### 4.6.3 Trust the Fates (Cost: 5 PP)
...
### 4.6.4 Shrug Off a Blow (Cost: 2 PP per point of damage negated)
...
### 4.6.5 Ignore a Fumble (Cost: 6 PP)
...
### 4.6.7 Stand Your Ground (Cost: 2 PP, then 2 PP more each consecutive round)
...
### 4.6.8 Deflect the Killing Blow (Cost: all remaining PP, minimum 8)
...
### 4.6.10 Rally From the Wound (Cost: 8 PP)
...
## 4.7 Experience and Improvement

### 4.7.1 Earning Experience Checkmarks
...
### 4.7.5 Characteristic Improvement
